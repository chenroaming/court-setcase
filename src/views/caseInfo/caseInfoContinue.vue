...
<div class="steps">
    ...
    <Icon type="chevron-down" class="setStep" v-show="isRight == true" :class="{'setActive':stepNum == 4}"></Icon>
...
<div class="content_main">
        ...
        <FormItem label="案件类别" style="width: 405px;">
            <Select v-model="caseType" transfer  placeholder="请选择">
                    <Option v-for="item in optionsCase" v-bind:value="item.value">{{item.text}}</Option>
                    <!-- <Option value=2 >执行案件</Option> -->
            </Select>
        </FormItem>
        <FormItem label="标的金额" style="width: 405px;position: relative;">
            <Input v-model="standardMoney" placeholder="标的金额"></Input> <span style="color:#495060;position: absolute;top: 2px;
right: -20px;">元</span>
        </FormItem>
    ...
    <div class="over_flo" v-show="liniAdd" style="height:450px">
        <div class="itemsd">
            ...
            <div v-for="(item,index) in liniList" v-if="item.typeStatus == '原告'" style="float:left">
                <div class="info matr">
                    <div class="bmbox">
    ...
    <div class="over_flo" v-show="fileAdd" style="height:450px;">
        ...
        <div class="boxContent">
            ...
            <p style="margin-top:10px;color:black">{{qfileName}}<span class="cansal" @click="cancelFile(3)">X</span></p>
        </div>
        <div class="boxContent" style="width: 195px;">
            ...
            <div class="add matrShen">
                <div v-show="qisuShow1" class="add matrFileShen" style="width:160px;"  @click="upFile(1)">
            ...
            <div class="add matrShen">
                <div  v-show="qisuShow2" class="add matrFileShen" style="width:160px;" @click="upFile(2)">
    ...
    <div>
        <Form :model="addFormItem" :label-width="100" inline>
            ...
            <FormItem label="电话" style="width: 245px;">
                ...
                <!-- <Dropdown @on-click="changePhone" v-if="this.lawyerId != ''">
                    <Input v-model="litigantPhoneSelect" :disabled="isDisabled" placeholder="请选择手机号" @on-change></Input>
                    <DropdownMenu slot="list">
                        <DropdownItem v-for="(item,index) in addFormItem.litigantPhone" :name="index">{{item}}</DropdownItem>
                    </DropdownMenu>
                </Dropdown>
                <Input v-model="litigantPhoneSelect" placeholder="请输入手机号" v-if="this.lawyerId == ''"></Input>
                <div style="color: #ed3f14;position:absolute;top:28px;" v-if="this.lawyerId == ''">多个手机号码请用逗号分隔</div> -->
            </FormItem>
            ...
            <FormItem label="工作证件号码*" v-show="!lawyerT1" style="width: 505px;">
                ...
                <AutoComplete v-model="addFormItem.lawerNum" :data="lawNumData1" @on-search="queryAgent1" placeholder="请输入代理人工作证件号码" transfer></AutoComplete>
            </FormItem>
...
v-clipboard:error="onError" style="font-size: 18px;margin-left: 10px;cursor: pointer;"/></p>
...
completeCase(){
    ...
    getOnlineLawCaseEdit(this.caseId).then(res => {
        if(res.data.state == 100){
            ...
            res.data.result.onlineLitigants.map(item => {
                if(item.litigationStatus.name=="原告"){
                    const data = {
                ...
                }else if(item.litigationStatus.name=="被告"){
                    const data = {
                        litigantName:item.litigantName,
                        identityCard:item.identityCard,
                        litigantPhone:item.litigantType == 0 ? item.litigantPhone : (item.litigantType == 1 ? item.legalManPhone : item.litigantTelPhone),
                ...
                if(item.onlineLawyers.length != 0){
                    const das = {
                        ...
                        litigantPhone:item.onlineLawyers[0].phone,
                        address:item.onlineLawyers[0].address,
                    }
    ...
    //         address:this.lawyerList[n].phone,
    //     }
    //     this.lawyerListSure.push(das);
    // }
...
lastSubmit(){
    ...
    // commitCae(this.caseId, this.mediate).then(res => {
    //     if(res.data.state == 100){
    //         this.code = res.data.code;
    //         this.caseSure = false;
    //         this.sqKnow = false;
...
getOneLini(id){ //获取当事人信息
    ...
    getOneLitigant(id).then(res => {
        if(res.data.state == 100){
            ...
            this.addFormItem.litigantPhone= res.data.onlineLitigant.litigantPhone ? res.data.onlineLitigant.litigantPhone.split(',') : ['请点击添加号码'];
                    ...
                    this.$Message.success('修改成功');
                        for(var i=0;i<this.lawyerList.length;i++){
                            if(this.lawyerList[i].id == this.lawyerId){
                                ...
                                if(this.lawyerT1 == false){ //律师
                                    this.lawyerList[i].card = this.addFormItem.lawerNum;                                        
                                }else{
                                    this.lawyerList[i].card = this.addFormItem.lawIdentiCard;
                                }
...
downLoadF(){
    // window.open("https://court1.ptnetwork001.com/api/online/evidenceAttachment/downFiles.jhtml")
